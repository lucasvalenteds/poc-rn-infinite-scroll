...
test('It returns 10 notifications by default', async () => {
    const response: AxiosResponse<{notifications: Notification[]}> = await httpClient.get('/');
...
test('It handles pagination via `offset` query parameter', async () => {
    const response: AxiosResponse<{notifications: Notification[]}> = await httpClient.get('/?offset=20');
